...
## Linhua Sun
## 20191028
## hicPlotDistVsCounts ==>> ThisScriptRePlot
...
cat(yellow(paste("Plot a single figure: ", opt$single,  sep = "")),sep = "\n")
###################################################################

# See whether these packages exist on comp. If not, install.
package_list <- c("data.table","ggplot2","scales","ggpmisc","ggsci","ggpubr","sitools")
...
CountsVsDist<-fread(opt$input,skip = "Matrix")[V2 !="Matrix"][]
...
if (opt$single=="single") {
    ...
    OutName <- paste(opt$out, "_AllChr.pdf", sep = "")
    #pdf(file = OutName,
    #    height = 6,
    #    width = 8)
    ...
    p2 <-
        ...
        pal_aaas(alpha = 0.8)(9)[3],
        pal_aaas(alpha = 0.8)(9)[5],
        pal_aaas(alpha = 0.8)(9)[1],
        pal_aaas(alpha = 0.8)(9)[2],
        pal_aaas(alpha = 0.8)(9)[6],
        pal_aaas(alpha = 0.8)(9)[8]
        ))
    ...
    OutName <- paste(opt$out, "_PerChr.pdf", sep = "")
