...
option_list = list(
    ...
    make_option(
        ...
        type = "logical",
        default = F,
        help = "whether remove countact counts from pericentromere (discarded)",
        metavar = "logical"
    )
)
...
KK<-fread("/data1/linhua/QIANLAB/PROJECT/hic/fragments/RF_SF_ID_Table.tsv",header = F,col.names = c("old","new")) ## Need to change it based on env.
...
inner_join(TXT, bed1, by = "name1") %>%
    ...
    fwrite(file = OutFileName,sep="\t",col.names = F,scipen = 50) %>%
    as.data.table() -> UU
...
if (Chr) {
    ...
    UU[seqnames1=="Chr1" & seqnames2=="Chr1"] %>% fwrite(file = paste(OutName,"_INT_Chr1",".tsv",sep=""),sep = "\t",col.names = F,nThread = 10,scipen = 50)
    UU[seqnames1=="Chr2" & seqnames2=="Chr2"] %>% fwrite(file = paste(OutName,"_INT_Chr2",".tsv",sep=""),sep = "\t",col.names = F,nThread = 10,scipen = 50)
    UU[seqnames1=="Chr3" & seqnames2=="Chr3"] %>% fwrite(file = paste(OutName,"_INT_Chr3",".tsv",sep=""),sep = "\t",col.names = F,nThread = 10,scipen = 50)
    UU[seqnames1=="Chr4" & seqnames2=="Chr4"] %>% fwrite(file = paste(OutName,"_INT_Chr4",".tsv",sep=""),sep = "\t",col.names = F,nThread = 10,scipen = 50)
    UU[seqnames1=="Chr5" & seqnames2=="Chr5"] %>% fwrite(file = paste(OutName,"_INT_Chr5",".tsv",sep=""),sep = "\t",col.names = F,nThread = 10,scipen = 50)
...
if (Chr) {
    SS[seqnames=="Chr1"] %>% fwrite(file = paste(OutName,"_Frag_Chr1",".tsv",sep=""),sep = "\t",col.names = F,scipen = 50)
    SS[seqnames=="Chr2"] %>% fwrite(file = paste(OutName,"_Frag_Chr2",".tsv",sep=""),sep = "\t",col.names = F,scipen = 50)
    SS[seqnames=="Chr3"] %>% fwrite(file = paste(OutName,"_Frag_Chr3",".tsv",sep=""),sep = "\t",col.names = F,scipen = 50)
    SS[seqnames=="Chr4"] %>% fwrite(file = paste(OutName,"_Frag_Chr4",".tsv",sep=""),sep = "\t",col.names = F,scipen = 50)
    SS[seqnames=="Chr5"] %>% fwrite(file = paste(OutName,"_Frag_Chr5",".tsv",sep=""),sep = "\t",col.names = F,scipen = 50)
